...
function AnalyticsDashboard({analytics,vulns}){
  ...
  return(
    <div id="avia-admin-print-root" style={{display:"flex",flexDirection:"column",gap:20}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div>
              ...
              <p style={{fontSize:13,color:"#94a3b8",lineHeight:1.7,maxWidth:600}}>
                Deliberate-engagement metrics derived from the tamper-evident audit log — vulnerability analyses, ISO/SAE 21434 compliance exports, and work product generated using Auto-VIA. Reach and visitor figures are reported independently via Google Analytics 4.
              </p>
            </div>
      ...
      {/* Work-product counts from the audit log. Reach/visitor/retention */}
      {/* figures are reported via GA4 and are not duplicated here. */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        <StatBox ico="📊" value={data.total_analyses||0} label="Vulnerability Analyses" color="var(--blue)" sub="Performed in-platform"/>
        ...
        <StatBox ico="➕" value={data.total_manual_assessments||0} label="Manual Assessments" color="#d97706" sub="Deliberate work product"/>
      </div>
      ...
      <Card pad="24px" style={{borderLeft:"4px solid var(--blue)",background:"var(--blue-bg)"}}>
        <div style={{fontSize:15,fontWeight:700,color:"var(--text-0)",lineHeight:1.8,fontStyle:"italic"}}>
          "{(data.total_analyses||0)>0?`${filterActive?`Between ${new Date(dateFrom).toLocaleDateString("en-US",{month:"long",year:"numeric"})} and ${new Date(dateTo).toLocaleDateString("en-US",{month:"long",year:"numeric"})}`:"To date"}, users performed ${(data.total_analyses||0).toLocaleString()} vulnerability analyses using the Auto-VIA platform${totalExports>0?`, generating ${totalExports.toLocaleString()} ISO/SAE 21434 compliance-ready exports (TARA asset registers and AVR records)`:""}.`:"Interact with the platform to begin recording work-product activity."}"
        </div>
      ...
      {/* ── DETAILED EXPORT & ENGAGEMENT METRICS ── */}
      {/* All counts sourced from the audit log. */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
...
PLATFORM ACTIVITY (FROM AUDIT LOG)
      ...
      <Card>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:20}}>
          {[
            ...
            ["📊","Layer 2: Google Analytics 4","GA4 (Measurement ID: G-L770P062V5) runs independently on Google's infrastructure via gtag.js. Every tracked event is dual-fired to both Layer 1 and GA4, so the two data sets can be cross-referenced for consistency in session, geographic, and event counts.","Cross-reference GA4 reporting with these metrics","#2563eb","#eff6ff"],
