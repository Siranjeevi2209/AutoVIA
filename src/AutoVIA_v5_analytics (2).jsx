...
function AnalyticsDashboard({analytics,vulns}){
  ...
  const uniqueCVEs=analytics.unique_cves_reviewed||analytics.cve_ids_viewed?.length||0;
      ...
      <Card>
        ...
        <div style={{padding:16,borderRadius:12,background:"var(--surface-2)",border:"1px solid var(--border)",marginBottom:16}}>
          ...
          <div style={{fontSize:12,color:"var(--text-1)",lineHeight:1.8}}>
            Include two exhibits side by side: (1) This platform's evidence report showing centralized analytics collected from all users worldwide via the Upstash backend, with a tamper-evident SHA-256 audit chain, and (2) Google Analytics 4 screenshots showing matching session counts and geographic data from Google's servers. When an immigration officer sees the same numbers from two independent systems — one built into the tool's backend infrastructure, one from Google — the data becomes effectively irrefutable. The hash-chained audit log provides additional cryptographic assurance that the internal data was recorded in real time and not retroactively fabricated.
          </div>
        </div>
      </Card>
