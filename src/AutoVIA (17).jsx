...
import { automotiveRelevance } from "./autovia_relevance_classifier";
...
const ECU = {
  ...
  adas:         { name: "ADAS",          full: "ADAS / Autonomous",     asil: "ASIL-B/D", mod: 1.20, computeMod: 1.30, color: "#7c3aed", bg: "#f5f3ff", ico: "🤖" },
  ...
  ev_charging:  { name: "EV Charging",   full: "EV Charging / Infrastructure", asil: "QM", mod: 1.00, color: "#0d9488", bg: "#f0fdfa", ico: "🔌" },
  unknown:      { name: "Unclassified",  full: "Unclassified / Non-ECU", asil: "QM",     mod: 1.00, color: "#94a3b8", bg: "#f8fafc", ico: "❓" },
};
...
// ═══════════════════════════════════════════════════════════════════
// ANALYTICS ENGINE — Platform Usage Tracking
// ═══════════════════════════════════════════════════════════════════
// Tracking runs in the background. The analytics dashboard is private
// and requires the ?admin=autovia URL parameter. Each event is appended
// to a tamper-evident audit log with SHA-256 hash chaining so usage
// figures can be independently verified against the underlying log.
//
// Data layers:
//   Layer 1 — Internal analytics (hash-chained, Upstash Redis)
//   Layer 2 — Google Analytics 4 (independent, G-L770P062V5)
//   Layer 3 — Returning-visitor tracking (localStorage visitor_id)
//
// BACKEND API CONTRACT (POST /api/analytics):
//   Body: { event, country, meta, visitor_id, is_returning, visit_count }
//   The backend MUST:
//   1. Increment daily_activity[today] in the Redis hash
//   2. Track visitor_id in a Redis set (SADD autovia:visitors <id>)
//   3. Increment returning_sessions or new_sessions based on is_returning
//   4. Append to audit_log with hash chain
//   Without #1, the daily activity chart shows flat/zero.
// ═══════════════════════════════════════════════════════════════════
...
// Generates a persistent visitor_id (UUID v4) stored in localStorage.
// First visit → new user. Subsequent visits → returning user.
// This is privacy-friendly (no PII), survives browser restarts,
// and gives us new vs returning segmentation.
function getVisitorInfo(){
  const STORAGE_KEY="autovia_visitor";
  try{
    const stored=localStorage.getItem(STORAGE_KEY);
    if(stored){
      const info=JSON.parse(stored);
      info.visit_count=(info.visit_count||1)+1;
    ...
    return{visitor_id:"anon-"+Date.now(),first_seen:new Date().toISOString(),
      last_seen:new Date().toISOString(),visit_count:1,is_returning:false};
  }
}

// ── GA4 Integration ──────────────────────────────────────────────
// Injects gtag.js and fires custom events to GA4 (G-L770P062V5).
// Runs independently of Layer 1 (Upstash) for dual-verification.
const GA4_MEASUREMENT_ID="G-L770P062V5";
...
function initGA4(){
  try{
    ...
    window.gtag("config",GA4_MEASUREMENT_ID,{send_page_views:true});
  }catch(e){console.warn("GA4 init failed:",e);}
}

function trackGA4(eventName,params={}){
  try{
    if(typeof window.gtag==="function"){
      window.gtag("event",eventName,{event_category:"autovia",...params});
    }
  }catch{}
}
...
function getAnalyticsDefaults(){
  return{
    ...
    unique_visitors:0, returning_sessions:0, new_sessions:0,
    visitor_ids:{}, // {visitor_id: {first_seen, last_seen, visit_count, country}}
...
function useAnalytics(){
  ...
  // Check if admin mode — fetch aggregated data from backend
  const isAdminMode=React.useMemo(()=>{
    try{return new URLSearchParams(window.location.search).get("admin")==="autovia";}catch{return false;}
  },[]);

  // Load aggregated analytics from backend (admin only)
  useEffect(()=>{
    if(!isAdminMode){loaded.current=true;return;}
    async function load(){
      try{
        const r=await fetch("/api/analytics-admin?secret=autovia");
        if(r.ok){const data=await r.json();setAnalytics(prev=>({...getAnalyticsDefaults(),...data}));}
      }catch{}
      loaded.current=true;
    }
    load();
    // Refresh every 30 seconds when admin is viewing
  ...
  const track=useCallback((event,meta={})=>{
    ...
    // Fire to GA4 (Layer 2) — independent verification
    trackGA4(event,{country,...meta,visitor_id:visitor.visitor_id,is_returning:visitor.is_returning});
    // Fire to backend (Layer 1) — Upstash with hash chain
    fetch("/api/analytics",{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({event,country,meta,
        visitor_id:visitor.visitor_id,
        is_returning:visitor.is_returning,
        visit_count:visitor.visit_count
      })
    }).catch(()=>{});
    // Also update local state if admin is viewing (for instant feedback)
    setAnalytics(prev=>{
      const next={...prev,last_seen:new Date().toISOString()};
      const today=new Date().toISOString().split("T")[0];
      const month=today.slice(0,7);
      if(!next.daily_activity)next.daily_activity={};
      if(!next.daily_activity[today])next.daily_activity[today]=0;
      next.daily_activity[today]++;
      if(!next.monthly_analyses)next.monthly_analyses={};
      if(!next.monthly_analyses[month])next.monthly_analyses[month]=0;
      if(!next.countries)next.countries={};
      if(country!=="XX"){if(!next.countries[country])next.countries[country]=0;next.countries[country]++;}
      // Track unique visitors and returning status
      if(!next.visitor_ids)next.visitor_ids={};
      if(!next.visitor_ids[visitor.visitor_id]){
        next.visitor_ids[visitor.visitor_id]={first_seen:visitor.first_seen,country};
        next.unique_visitors=(next.unique_visitors||0)+1;
      }
      next.visitor_ids[visitor.visitor_id].last_seen=new Date().toISOString();
      next.visitor_ids[visitor.visitor_id].visit_count=visitor.visit_count;
      switch(event){
        case"session_start":
          next.total_sessions=(next.total_sessions||0)+1;
          if(visitor.is_returning){next.returning_sessions=(next.returning_sessions||0)+1;}
          else{next.new_sessions=(next.new_sessions||0)+1;}
          break;
        case"analysis_complete":next.total_analyses=(next.total_analyses||0)+1;next.monthly_analyses[month]++;break;
        case"cve_view":next.total_cve_views=(next.total_cve_views||0)+1;break;
        case"avr_download":next.total_avr_downloads=(next.total_avr_downloads||0)+1;break;
        case"tara_export":next.total_tara_exports=(next.total_tara_exports||0)+1;break;
        case"csv_export":next.total_csv_exports=(next.total_csv_exports||0)+1;break;
        case"json_export":next.total_json_exports=(next.total_json_exports||0)+1;break;
        case"ai_query":next.total_ai_queries=(next.total_ai_queries||0)+1;break;
        case"live_search":next.total_live_searches=(next.total_live_searches||0)+1;break;
        case"manual_assess":next.total_manual_assessments=(next.total_manual_assessments||0)+1;break;
        case"view_change":
          if(!next.feature_usage)next.feature_usage={};
          if(meta.view){next.feature_usage[meta.view]=(next.feature_usage[meta.view]||0)+1;}break;
        case"ecu_analyzed":
          if(!next.ecu_domains_analyzed)next.ecu_domains_analyzed={};
          if(meta.domain){next.ecu_domains_analyzed[meta.domain]=(next.ecu_domains_analyzed[meta.domain]||0)+1;}break;
        case"peak_avrs":
          if(meta.count>(next.peak_concurrent_avrs||0))next.peak_concurrent_avrs=meta.count;break;
      }
      return next;
    });
  },[]);
  ...
  const resetAnalytics=useCallback(async()=>{
    setAnalytics(getAnalyticsDefaults());
  },[]);
...
function AnalyticsDashboard({analytics,vulns}){
  ...
  // Compute filtered metrics from audit log when date range is active
  const filtered=useMemo(()=>{
    if(!filterActive||!dateFrom||!dateTo)return null;
    const from=dateFrom;const to=dateTo;
    const logs=(analytics.audit_log||[]).filter(e=>{
      const d=e.ts?.split("T")[0];return d&&d>=from&&d<=to;
    });
    // Recompute everything from filtered audit log
    const m={
      total_sessions:0,total_analyses:0,total_cve_views:0,total_avr_downloads:0,
      total_tara_exports:0,total_csv_exports:0,total_json_exports:0,total_ai_queries:0,
      total_live_searches:0,total_manual_assessments:0,
      countries:{},daily_activity:{},monthly_analyses:{},ecu_domains_analyzed:{},
      feature_usage:{},priority_tiers_viewed:{}
    };
    logs.forEach(e=>{
      const day=e.ts?.split("T")[0];const month=day?.slice(0,7);
      if(day){if(!m.daily_activity[day])m.daily_activity[day]=0;m.daily_activity[day]++;}
      if(month){if(!m.monthly_analyses[month])m.monthly_analyses[month]=0;m.monthly_analyses[month]++;}
      if(e.co&&e.co!=="XX"){if(!m.countries[e.co])m.countries[e.co]=0;m.countries[e.co]++;}
      switch(e.ev){
        case"session_start":m.total_sessions++;break;
        case"analysis_complete":m.total_analyses++;break;
        case"cve_view":m.total_cve_views++;break;
        case"avr_download":m.total_avr_downloads++;break;
        case"tara_export":m.total_tara_exports++;break;
        case"csv_export":m.total_csv_exports++;break;
        case"json_export":m.total_json_exports++;break;
        case"ai_query":m.total_ai_queries++;break;
        case"live_search":m.total_live_searches++;break;
        case"manual_assess":m.total_manual_assessments++;break;
      }
    });
    return m;
  },[analytics.audit_log,dateFrom,dateTo,filterActive]);

  // Use filtered data if active, otherwise use totals
  const data=filtered||analytics;
  const totalExports=(data.total_avr_downloads||0)+(data.total_tara_exports||0)+(data.total_csv_exports||0)+(data.total_json_exports||0);
  const countryCount=Object.keys(data.countries||{}).length;
  const uniqueCVEs=filtered?data.total_cve_views:(analytics.unique_cves_reviewed||analytics.cve_ids_viewed?.length||0);
  const periodStart=filterActive&&dateFrom?dateFrom:analytics.first_seen;
  const periodEnd=filterActive&&dateTo?dateTo:analytics.last_seen;
  const daysSinceLaunch=Math.max(1,Math.ceil((new Date(periodEnd)-new Date(periodStart))/(1000*60*60*24)));

  // Tracked sessions = only sessions since visitor tracking was deployed
  // Uses new_sessions + returning_sessions (not historical total_sessions)
  const trackedSessions=(data.returning_sessions||0)+(data.new_sessions||0);
  const retentionPct=trackedSessions>0?((data.returning_sessions||0)/trackedSessions*100):0;

  // Top ECU domains
  const topECUs=Object.entries(data.ecu_domains_analyzed||{}).sort((a,b)=>b[1]-a[1]);
  ...
  const dateInputStyle={background:"var(--surface-2)",border:"1px solid var(--border)",borderRadius:10,padding:"8px 12px",color:"var(--text-0)",fontFamily:"var(--mono)",fontSize:12,outline:"none"};

  return(
    <div id="avia-admin-print-root" style={{display:"flex",flexDirection:"column",gap:20}}>
      <style>{`
        @media print {
          @page { size: A4 portrait; margin: 14mm; }
          body { background: #fff !important; }
          .avia-no-print { display: none !important; }
          #avia-admin-print-root { gap: 14px !important; }
          #avia-admin-print-root * {
            -webkit-print-color-adjust: exact !important;
            print-color-adjust: exact !important;
          }
          /* Avoid splitting cards across pages where possible */
          #avia-admin-print-root > div { break-inside: avoid; page-break-inside: avoid; }
        }
      `}</style>
      {/* Print / export toolbar — hidden when printing */}
      <div className="avia-no-print" style={{display:"flex",justifyContent:"flex-end",gap:8}}>
        <button onClick={()=>window.print()} style={{padding:"8px 16px",borderRadius:10,background:"var(--blue)",border:"none",color:"#fff",fontFamily:"var(--font)",fontSize:12,fontWeight:700,cursor:"pointer"}}>🖨 Print / Save as PDF</button>
      </div>
      {/* Date Range Filter */}
      <Card className="avia-no-print" pad="16px" style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
        <div style={{fontSize:13,fontWeight:700,color:"var(--text-0)"}}>📅 Filter by Date Range</div>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <label style={{fontSize:11,color:"var(--text-2)",fontWeight:600}}>From</label>
          <input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} style={dateInputStyle}/>
          <label style={{fontSize:11,color:"var(--text-2)",fontWeight:600}}>To</label>
          <input type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)} style={dateInputStyle}/>
        </div>
        <button onClick={()=>{if(dateFrom&&dateTo)setFilterActive(true);}} disabled={!dateFrom||!dateTo}
          style={{padding:"8px 16px",borderRadius:10,background:dateFrom&&dateTo?"var(--blue)":"var(--surface-2)",border:dateFrom&&dateTo?"none":"1px solid var(--border)",color:dateFrom&&dateTo?"#fff":"var(--text-3)",fontFamily:"var(--font)",fontSize:12,fontWeight:700,cursor:dateFrom&&dateTo?"pointer":"default",transition:"all .2s"}}>Apply Filter</button>
        {filterActive&&<button onClick={()=>{setFilterActive(false);setDateFrom("");setDateTo("");}}
          style={{padding:"8px 16px",borderRadius:10,background:"#fff1f2",border:"1px solid #fecdd3",color:"#e11d48",fontFamily:"var(--font)",fontSize:12,fontWeight:700,cursor:"pointer"}}>✕ Clear Filter</button>}
        {filterActive&&<Tag color="#059669" bg="#ecfdf5" style={{fontWeight:700}}>Showing: {dateFrom} → {dateTo}</Tag>}
      </Card>
      {/* Header Banner */}
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div>
              <div style={{display:"flex",gap:8,marginBottom:12}}>
                ...
                <span style={{padding:"4px 12px",borderRadius:20,fontSize:10,fontWeight:700,background:"rgba(37,99,235,0.2)",color:"#93c5fd",border:"1px solid rgba(37,99,235,0.3)"}}>HASH-VERIFIED</span>
              </div>
              <h2 style={{fontFamily:"var(--font)",fontSize:24,fontWeight:800,color:"#fff",marginBottom:6}}>Platform Engagement Analytics</h2>
              <p style={{fontSize:13,color:"#94a3b8",lineHeight:1.7,maxWidth:600}}>
                Work-product activity computed from the tamper-evident audit log — ISO/SAE 21434 compliance exports, manual risk assessments, and analysis events by ECU domain generated using Auto-VIA.
              </p>
            </div>
      ...
      {/* Work-product counts from the audit log. */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
        <StatBox ico="📦" value={totalExports} label="Compliance Exports" color="#ea580c" sub="TARA + AVR + CSV + JSON"/>
        <StatBox ico="➕" value={data.total_manual_assessments||0} label="Manual Assessments" color="#d97706" sub="Analyst-authored work product"/>
      ...
      <Card pad="24px" style={{borderLeft:"4px solid var(--blue)",background:"var(--blue-bg)"}}>
        <div style={{fontSize:15,fontWeight:700,color:"var(--text-0)",lineHeight:1.8,fontStyle:"italic"}}>
          "{totalExports>0?`${filterActive?`Between ${new Date(dateFrom).toLocaleDateString("en-US",{month:"long",year:"numeric"})} and ${new Date(dateTo).toLocaleDateString("en-US",{month:"long",year:"numeric"})}`:"To date"}, users generated ${totalExports.toLocaleString()} ISO/SAE 21434 compliance-ready exports (TARA asset registers and AVR records)${(data.total_manual_assessments||0)>0?` and authored ${(data.total_manual_assessments||0).toLocaleString()} manual risk assessments`:""} using the Auto-VIA platform.`:"Interact with the platform to begin recording work-product activity."}"
        </div>
        <div style={{fontSize:11,color:"var(--text-2)",marginTop:8}}>↑ Computed from the tamper-evident audit log{filterActive?" (filtered by date range)":""}.</div>
      </Card>

      {/* ── DETAILED EXPORT METRICS ── */}
      {/* Primary compliance outputs (audit-log sourced). */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        <StatBox ico="📋" value={data.total_tara_exports||0} label="TARA Exports" color="#059669"/>
        <StatBox ico="📥" value={data.total_avr_downloads||0} label="AVR Downloads" color="#0891b2"/>
        <StatBox ico="📊" value={data.total_csv_exports||0} label="CSV Exports" color="#ea580c" sub="Secondary"/>
        <StatBox ico="📦" value={data.total_json_exports||0} label="JSON Exports" color="#7c3aed" sub="Secondary"/>
      </div>
      {/* Secondary in-tool activity — shown live, excluded from the printed report */}
      <div className="avia-no-print" style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12}}>
        <StatBox ico="🤖" value={data.total_ai_queries||0} label="AI Analysis Queries" color="#7c3aed"/>
        <StatBox ico="🔎" value={data.total_live_searches||0} label="Live NVD Searches" color="var(--blue)"/>
      </div>



      {/* ── ANALYSIS EVENTS BY ECU DOMAIN ── */}
      <Card>
        <CardTitle icon="🏎️">Analysis Events by ECU Domain</CardTitle>
        <p style={{fontSize:11,color:"var(--text-3)",marginBottom:14}}>Distribution of analysis events across vehicle subsystems — shows automotive-specific coverage.</p>
        {topECUs.length===0?<div style={{padding:16,textAlign:"center",color:"var(--text-3)",fontSize:12}}>No domain-specific analysis events recorded yet.</div>:
        topECUs.map(([domain,count],i)=>{const e=ECU[domain];const pct=(count/maxECU)*100;return(
          <div key={domain} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            <span style={{minWidth:160,fontSize:12,fontWeight:600,color:e?.color||"var(--text-2)"}}>{e?.ico} {e?.name||domain}</span>
      ...
      <Card pad="24px" style={{background:"linear-gradient(135deg,#f8fafc,#f1f5f9)",border:"1px solid var(--border)"}}>
        <CardTitle icon="📋">Platform Usage Report</CardTitle>
        <p style={{fontSize:12,color:"var(--text-2)",marginBottom:16}}>Structured summary of platform usage, computed from the audit log, with a downloadable report.{filterActive?" Showing filtered data for selected date range.":""}</p>
...
`AUTO-VIA PLATFORM USAGE REPORT
${"═".repeat(50)}
Generated: ${new Date().toLocaleString("en-US",{year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit",timeZoneName:"short"})}
Reporting Period: ${filterActive?dateFrom:new Date(analytics.first_seen).toLocaleDateString()} — ${filterActive?dateTo:new Date(analytics.last_seen).toLocaleDateString()} (${daysSinceLaunch} days)${filterActive?"\n⚠ FILTERED VIEW — showing data for selected date range only":""}

VERIFICATION
  All figures below are computed from a tamper-evident audit log
  secured by a SHA-256 hash chain. Modifying, inserting, or deleting
  any entry breaks the chain, allowing the totals to be independently
  re-derived and verified against the log.
  Audit log entries: ${(analytics.audit_log?.length||0).toLocaleString()}
  Chain head (SHA-256): ${(analytics.audit_chain_head||"—")}
  Independent secondary source: Google Analytics 4 (G-L770P062V5)

COMPLIANCE & EXPORT ACTIVITY
  Total Compliance Exports:         ${totalExports.toLocaleString()}
    ├─ TARA Asset Register Exports: ${(data.total_tara_exports||0).toLocaleString()}
    ├─ AVR Record Downloads:        ${(data.total_avr_downloads||0).toLocaleString()}
    ├─ CSV Data Exports (secondary):${(data.total_csv_exports||0).toLocaleString()}
    └─ JSON Data Exports (secondary):${(data.total_json_exports||0).toLocaleString()}

WORK PRODUCT
  Manual Risk Assessments Authored: ${(data.total_manual_assessments||0).toLocaleString()}
  Unique CVEs Reviewed:             ${(uniqueCVEs||0).toLocaleString()}

ANALYSIS EVENTS BY ECU DOMAIN
${topECUs.length?topECUs.map(([d,c])=>`  ${(ECU[d]?.full||d).padEnd(28)}${c.toLocaleString()} events`).join("\n"):"  (No domain-specific analysis events recorded yet)"}`
          }</pre>
        </div>
        <div className="avia-no-print" style={{display:"flex",gap:8,marginTop:14}}>
          <button onClick={()=>{
            const blob=new Blob([document.querySelector("[data-evidence-export]")?.textContent||""],{type:"text/plain"});
            const u=URL.createObjectURL(blob);const a=document.createElement("a");a.href=u;a.download=`AutoVIA_Usage_Report_${new Date().toISOString().split("T")[0]}.txt`;a.click();URL.revokeObjectURL(u);
          }} style={{padding:"10px 16px",borderRadius:10,background:"var(--blue)",border:"none",color:"#fff",fontFamily:"var(--mono)",fontSize:12,fontWeight:600,cursor:"pointer"}}>⬇ Export Usage Report (.txt)</button>
          <button onClick={()=>{
      ...
      <Card>
        ...
        <p style={{fontSize:12,color:"var(--text-2)",marginBottom:16}}>Usage data is recorded across two independent layers so figures can be verified against each other.</p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:20}}>
          {[
            ["🔗","Layer 1: Centralized Analytics with Hash-Chained Audit Log","Every user event from every browser is sent to a central Upstash Redis database via serverless API. Each event is appended to a SHA-256 hash chain — if any entry is modified, inserted, or deleted, the chain breaks, providing a cryptographic record of data integrity.",analytics.audit_log?.length?`${analytics.audit_log.length} entries · Chain head: ${(analytics.audit_chain_head||"").slice(0,12)}…`:"Chain not yet started","#7c3aed","#f5f3ff"],
            ["📊","Layer 2: Google Analytics 4","GA4 (Measurement ID: G-L770P062V5) runs independently on Google's infrastructure via gtag.js, providing a secondary independent record of platform activity alongside the audit log.","Independent secondary record","#2563eb","#eff6ff"],
...
const CPE_RULES = [
  [/\bev charg|electric vehicle charg|charging station|\bevse\b|\bocpp\b|\biocharger\b|\bsteve\b|\bchargepoint\b|\bwallbox\b/i,"ev_charging"],
  // Specific SAFETY-CRITICAL function/hardware signals first (spelled-out; collision-prone acronyms omitted)
  [/\bbrake\b|\bbraking\b|brake[\s-]?by[\s-]?wire|anti[\s-]?lock brak/i,"braking"],
  [/\bsteering\b|steer[\s-]?by[\s-]?wire|electric power steering/i,"steering"],
  [/\bairbag\b|restraint system|suspension control|active suspension|electronic stability|stability control|\besc\b/i,"chassis"],
  [/engine control|\bpowertrain\b|\bdrivetrain\b|transmission control|\binverter\b|battery management|on[\s-]?board charger|motor control unit/i,"powertrain"],
  // ADAS / autonomous
  [/\badas\b|advanced driver|driver assistance|lane keeping|lane departure|adaptive cruise|autonomous driving|self[\s-]?driving|automated driving|traffic sign recognition|blind spot|sensor fusion|\blidar\b|\baeb\b|\bmobileye\b|nvidia drive|baidu apollo|autoware|openpilot/i,"adas"],
  // Telematics / off-board connectivity
  [/\btelematics\b|\btcu\b|\bv2x\b|\bv2v\b|\bv2i\b|\bdsrc\b|\becall\b|e[\s-]?call|connected car|connected vehicle|\bgeotab\b|\bfleet\b|\bcellular\b|\b5g\b|\blte\b|over[\s-]the[\s-]air|\bota\b/i,"telematics"],
  // Infotainment / cockpit (incl. Harman audio, CarPlay, Android Automotive)
  [/\binfotainment\b|\bivi\b|head unit|digital cockpit|instrument cluster|android[\s-]?auto|android automotive|apple carplay|\bcarplay\b|\bharman\b/i,"infotainment"],
  // Body / comfort / access
  [/body control|\bbcm\b|door lock|\bkeyless\b|key fob|\bimmobili[sz]er\b|\btpms\b|tire pressure|\bhvac\b|climate control|power window|seat control/i,"body"],
  // Diagnostics
  [/\bdoip\b|unified diagnostic|\buds\b|\bobd(?:-?ii)?\b|diagnostic trouble|\bj2534\b/i,"diagnostics"],
  // In-vehicle network protocols -> gateway (the routing ECU)
  [/\bcan[\s-]?bus\b|\bcanbus\b|controller area network|\bsocketcan\b|\bj1939\b|\bflexray\b|\bsome\/ip\b|\bsomeip\b|automotive ethernet|gateway ecu|central gateway|zonal controller/i,"gateway"],
  // Weak connectivity signals -> dominant domain (lowest priority)
  [/\bbluetooth\b|\bble\b|\bwi-?fi\b/i,"infotainment"],
  // Shared components (QNX, VxWorks, AUTOSAR, Linux, OpenSSL, generic RTOSes), broad chip/supplier
  // vendors (Bosch, Continental, Denso, NXP, Infineon, Renesas) and bare carmaker names are
  // intentionally NOT mapped here: their ECU domain isn't determinable from the name, so they
  // fall through to "unknown" (QM, x1.00) instead of receiving an invented safety-critical domain.
];
...
// ── ENGINE ────────────────────────────────────────────────────────
function round1HalfUp(x){ return Math.round((x + Number.EPSILON) * 10) / 10; }
function computeARS(v) {
  const b=v.cvss_v4_base_score||v.cvss_base_score||0,e=ECU[v.ecu_domain];
  // ADAS two-tier rule: autonomous-compute platforms use ASIL-D (computeMod x1.30);
  // general ADAS defaults to ASIL-B/D (mod x1.20). Other domains use their single mod.
  const am=e?((v.ecu_domain==="adas"&&v.adas_tier==="compute"&&e.computeMod)?e.computeMod:e.mod):1;
  const rm=REACH[`${v.attack_surface}|${v.network_path}`]||1,em=EXPLOIT[v.exploit_maturity]||.9;
  // Canonical order of operations: multiply, CAP first, ROUND last (1 decimal, half-up).
  const raw=b*am*rm*em,score=round1HalfUp(Math.min(10,raw));
  let tier=v.kev_listed?"P0_critical":score>=9?"P0_critical":score>=7?"P1_high":score>=4?"P2_medium":"P3_low";
  const asilLabel=(v.ecu_domain==="adas")?(v.adas_tier==="compute"?"ASIL-D (autonomous-compute)":"ASIL-B/D (general)"):(e?.asil||"QM");
  return{ars:score,priority_tier:tier,recommended_action:TIERS[tier].act,raw_score:raw,asil_mod:am,reach_mod:rm,exploit_mod:em,
    justification_trace:[`CVSS Base: ${b}`,`ECU: ${e?.full||v.ecu_domain} (${asilLabel}) — ×${am}`,`Reach: ${v.attack_surface}/${v.network_path} — ×${rm}`,
      `Exploit: ${v.exploit_maturity} — ×${em}`,`Raw: ${raw.toFixed(3)}`,`ARS: ${score}`,v.kev_listed?"⚡ KEV → P0":`Tier: ${tier}`]};
}
function classify(text){for(const[p,d]of CPE_RULES)if(p.test(text))return d;return"unknown";}
function inferSurface(vec,desc){let s="local",p="unknown";const d=(desc||"").toLowerCase();if(vec?.includes("AV:N"))s="remote_external";else if(vec?.includes("AV:A"))s="remote_adjacent";else if(vec?.includes("AV:P"))s="physical";if(d.match(/\bcellular\b|\bota\b|\btelematics\b|\b5g\b|\blte\b|\bv2x\b/))p="telematics";else if(d.match(/\bbluetooth\b|\bble\b|\bwi-?fi\b|\bwireless\b|\bnfc\b|\buwb\b/))p="wifi_bt";else if(d.match(/\bethernet\b|\bsome\/ip\b|\bsomeip\b/))p="ethernet";else if(d.match(/\bcan[\s-]?bus\b|\bcanbus\b|controller area network|\bj1939\b|\bsocketcan\b|\bflexray\b/))p="can";else if(d.match(/\bdiagnostic\b|\buds\b|\bdoip\b|\bjtag\b|\bobd\b/))p="diagnostic";else if(s==="remote_external")p="telematics";else if(s==="remote_adjacent")p="wifi_bt";else if(s==="physical")p="diagnostic";else p="can";return{s,p};}
function parseNVD(item,kevSet){const c=item.cve;if(!c?.id)return null;const desc=c.descriptions?.find(d=>d.lang==="en")?.value||"";const m=c.metrics||{};let bs=0,vec="";if(m.cvssMetricV40?.length){bs=m.cvssMetricV40[0].cvssData?.baseScore||0;vec=m.cvssMetricV40[0].cvssData?.vectorString||"";}else if(m.cvssMetricV31?.length){bs=m.cvssMetricV31[0].cvssData?.baseScore||0;vec=m.cvssMetricV31[0].cvssData?.vectorString||"";}else if(m.cvssMetricV30?.length){bs=m.cvssMetricV30[0].cvssData?.baseScore||0;vec=m.cvssMetricV30[0].cvssData?.vectorString||"";}else if(m.cvssMetricV2?.length){bs=m.cvssMetricV2[0].cvssData?.baseScore||0;vec=m.cvssMetricV2[0].cvssData?.vectorString||"";}if(!bs)return null;const cpes=[];(c.configurations||[]).forEach(x=>(x.nodes||[]).forEach(n=>(n.cpeMatch||[]).forEach(m=>m.criteria&&cpes.push(m.criteria))));const cwes=[];(c.weaknesses||[]).forEach(w=>(w.description||[]).forEach(d=>d.value&&!d.value.includes("noinfo")&&!d.value.includes("Other")&&cwes.push(d.value)));const dom=classify(desc+" "+cpes.join(" "));const rel=automotiveRelevance(cpes,desc);const{s,p}=inferSurface(vec,desc);const kev=kevSet.has(c.id);let prod="";if(cpes.length){const x=cpes[0].split(":");if(x.length>=5)prod=`${x[3]}:${x[4]}${x[5]&&x[5]!=="*"?":"+x[5]:""}`;}if(!prod){const mm=desc.match(/(?:in|of|for)\s+([A-Z][\w\s.-]+?)(?:\s+(?:before|prior|through|allows|could|may|is|via))/i);prod=mm?mm[1].trim():"";}const v={cve_id:c.id,published:c.published?.split("T")[0]||"",modified:c.lastModified?.split("T")[0]||"",description:desc,cvss_v4_base_score:bs,cvss_vector:vec,cwe_ids:cwes,cpe_matches:cpes.slice(0,5),affected_product:prod,ecu_domain:dom,attack_surface:s,network_path:p,exploit_maturity:kev?"active_exploitation":"unknown",kev_listed:kev,source_feeds:kev?["NVD","CISA_KEV"]:["NVD"],classification_method:"cpe_taxonomy_rule",relevance_score:rel.p_auto,p_cpe:rel.p_cpe,p_keyword:rel.p_keyword,is_automotive:rel.is_automotive};return{...v,...computeARS(v)};}
...
function ManualInput({onCompute}){
  ...
  return(<Card style={{maxWidth:900,margin:"0 auto"}}><CardTitle icon="➕">Manual CVE Assessment</CardTitle>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>
      ...
      {f.ecu_domain==="adas"&&<div><label style={lbl}>ADAS Tier</label><select style={{...inp,cursor:"pointer"}} value={f.adas_tier||"general"} onChange={e=>u("adas_tier",e.target.value)}><option value="general">General ADAS (ASIL-B/D ×1.20)</option><option value="compute">Autonomous-compute / safety RTOS (ASIL-D ×1.30)</option></select></div>}
...
export default function AutoVIA(){
  ...
  // Initialize GA4 + Track session start
  useEffect(()=>{initGA4();track("session_start");},[]);// eslint-disable-line
  ...
  useEffect(()=>{let stop=false;async function load(){log("Starting…");
    try{const r=await fetch("/cve-database.json");if(r.ok){const db=await r.json();if(db.vulnerabilities?.length){
      ...
      const enriched=db.vulnerabilities.map(v=>{const bs=v.cvss_base_score||v.cvss_v4_base_score||0;const desc=v.description||"";const cpes=v.cpe_matches||[];const dom=classify(desc+" "+cpes.join(" "));const{s,p}=inferSurface(v.cvss_vector,desc);const rel=automotiveRelevance(cpes,desc);const vv={...v,cvss_v4_base_score:bs,ecu_domain:dom,attack_surface:s,network_path:p,relevance_score:rel.p_auto,p_cpe:rel.p_cpe,p_keyword:rel.p_keyword,is_automotive:rel.is_automotive};return{...vv,...computeARS(vv)};}).filter(v=>v.is_automotive);
  ...
  const liveSearch=async(q)=>{if(!q||q.length<3||liveSearching)return;setLiveSearching(true);track("live_search",{query:q});
    try{const r=await fetch(`${API_PROXY}?source=nvd&keyword=${encodeURIComponent(q)}&resultsPerPage=50`);if(!r.ok){setLiveSearching(false);return;}const d=await r.json();let a=0;
      setVulns(p=>{const m=new Map(p.map(v=>[v.cve_id,v]));for(const it of d.vulnerabilities||[]){const id=it.cve?.id;if(!id||m.has(id))continue;const av=parseNVD(it,kevSet);if(av&&av.cvss_v4_base_score>0&&av.is_automotive){m.set(id,av);a++;}}return[...m.values()].sort((a,b)=>b.ars-a.ars);});
...
ASIL MODIFIERS: ASIL-D(braking, steering, ADAS-autonomous-compute)=×1.30, ASIL-C(powertrain,chassis)=×1.20, ASIL-B/D(ADAS-general, default for ADAS)=×1.20, ASIL-B(gateway)=×1.10, ASIL-A(telematics)=×1.05, QM(infotainment,body,diagnostics)=×1.00. NOTE: ADAS uses a two-tier rule — autonomous-compute platforms and safety-certified RTOS (e.g., Green Hills INTEGRITY, QNX in ADAS roles) are ASIL-D ×1.30; general ADAS components default to ASIL-B/D ×1.20.
