...
//   Layer 1 — This internal analytics (hidden, hash-chained via Upstash)
//   Layer 2 — Google Analytics 4 (independent third party, G-L770P062V5)
//   Layer 3 — Returning user tracking (localStorage visitor_id)
//   Presenting all three with matching numbers = strong evidence.
//
...
//   4. Append to audit_log with hash chain as before
...
// and gives us new vs returning segmentation for EB-1A evidence.
...
function AnalyticsDashboard({analytics,vulns}){
  ...
  const avgDaily=((data.total_analyses||0)/daysSinceLaunch).toFixed(1);
  ...
  // Monthly trend data
  const monthlyEntries=Object.entries(data.monthly_analyses||{}).sort((a,b)=>a[0].localeCompare(b[0])).slice(-12);
  const maxMonthly=Math.max(...monthlyEntries.map(([,v])=>v),1);

  // Daily trend for date range or last 30 days
  const dailyDays=[];
  if(filterActive&&dateFrom&&dateTo){
    const start=new Date(dateFrom+"T00:00:00Z");const end=new Date(dateTo+"T00:00:00Z");
    for(let d=new Date(start);d<=end;d.setDate(d.getDate()+1)){const k=d.toISOString().split("T")[0];dailyDays.push({date:k,count:(data.daily_activity||{})[k]||0});}
  }else{for(let i=29;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);const k=d.toISOString().split("T")[0];dailyDays.push({date:k,count:(data.daily_activity||{})[k]||0});}}
  const maxDaily=Math.max(...dailyDays.map(d=>d.count),1);

  // Top countries
  const topCountries=Object.entries(data.countries||{}).sort((a,b)=>b[1]-a[1]).slice(0,15);
  ...
  // Feature usage
  const featureEntries=Object.entries(data.feature_usage||{}).sort((a,b)=>b[1]-a[1]);
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      {/* Date Range Filter */}
      <Card pad="16px" style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12}}>
        <StatBox ico="📊" value={data.total_analyses||0} label="Vulnerability Analyses" color="var(--blue)" sub={`${avgDaily}/day avg`}/>
        <StatBox ico="🌍" value={countryCount} label="Countries Reached" color="#059669" sub={countryCount>=5?"Global adoption":"Growing"}/>
        <StatBox ico="👥" value={data.total_sessions||0} label="User Sessions" color="#7c3aed" sub={`${daysSinceLaunch} days tracked`}/>
      ...
      {/* ── RETURNING USER METRICS ── */}
      {/* Only show once there's enough data — avoids awkward mismatch with historical totals */}
      {((Object.keys(data.visitor_ids||{}).length||(data.unique_visitors||0))>=20)&&<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        <StatBox ico="🆔" value={Object.keys(data.visitor_ids||{}).length||(data.unique_visitors||0)} label="Unique Visitors" color="#059669" sub="Distinct browser identities"/>
        <StatBox ico="🔁" value={data.returning_sessions||0} label="Returning Sessions" color="#7c3aed" sub={`${retentionPct.toFixed(0)}% return rate`}/>
        <StatBox ico="🆕" value={data.new_sessions||0} label="New User Sessions" color="#2563eb" sub="First-time visitors"/>
        <StatBox ico="📈" value={trackedSessions>0?retentionPct.toFixed(0)+"%":"—"} label="Retention Rate" color={retentionPct>=30?"#059669":"#d97706"} sub={retentionPct>=30?"Strong stickiness":"Building engagement"}/>
      </div>}
      ...
      <Card pad="24px" style={{borderLeft:"4px solid var(--blue)",background:"var(--blue-bg)"}}>
        <div style={{fontSize:15,fontWeight:700,color:"var(--text-0)",lineHeight:1.8,fontStyle:"italic"}}>
          "{(data.total_sessions||0)>0?`${filterActive?`Between ${new Date(dateFrom).toLocaleDateString("en-US",{month:"long",year:"numeric"})} and ${new Date(dateTo).toLocaleDateString("en-US",{month:"long",year:"numeric"})}`:`Within ${daysSinceLaunch} day${daysSinceLaunch!==1?"s":""} of deployment`}, ${(data.total_analyses||0)>0?"engineers and researchers":"users"} ${countryCount>1?`across ${countryCount} countries `:""} performed ${(data.total_analyses||0).toLocaleString()} vulnerability analyses using the Auto-VIA platform${totalExports>0?`, generating ${totalExports.toLocaleString()} compliance-ready exports`:""}${retentionPct>=10&&(data.unique_visitors||Object.keys(data.visitor_ids||{}).length||0)>=20?`. ${retentionPct.toFixed(0)}% of sessions were from returning users, demonstrating sustained utility beyond initial curiosity`:""}.`:"Launch the platform and interact with it to begin tracking metrics."}"
        </div>
        <div style={{fontSize:11,color:"var(--text-2)",marginTop:8}}>↑ Auto-generated evidence statement — updates in real time as usage grows{filterActive?" (filtered by date range)":""}</div>
      </Card>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        ...
        <StatBox ico="➕" value={data.total_manual_assessments||0} label="Manual Assessments" color="#d97706"/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        ...
        <StatBox ico="📊" value={data.total_csv_exports||0} label="CSV Exports" color="#ea580c"/>
        <StatBox ico="📦" value={data.total_json_exports||0} label="JSON Exports" color="#7c3aed"/>
        <StatBox ico="🏎️" value={filterActive?"—":(analytics.peak_concurrent_avrs||0)} label="Peak AVRs Processed" color="#e11d48"/>
      </div>
      ...
      <Card>
        <CardTitle icon="📈">{filterActive?`Daily Activity — ${dateFrom} to ${dateTo}`:"Daily Activity Trend — Last 30 Days"}</CardTitle>
        <p style={{fontSize:12,color:"var(--text-2)",marginBottom:16}}>{filterActive?"Filtered activity within the selected date range.":"Platform engagement over time. Consistent activity demonstrates sustained utility and adoption beyond initial launch."}</p>
        <div style={{display:"flex",alignItems:"flex-end",gap:2,height:120,padding:"0 4px"}}>
          {(()=>{const logMax=Math.log(maxDaily+1);return dailyDays.map((d,i)=>{const h=d.count>0?Math.max(8,(Math.log(d.count+1)/logMax)*100):2;return(
            <div key={d.date} title={`${d.date}: ${d.count} events`} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end",gap:4,animation:`fadeUp .3s ease ${i*.02}s both`}}>
              {d.count>0&&<div style={{fontSize:8,fontFamily:"var(--mono)",color:"var(--text-3)",lineHeight:1}}>{d.count}</div>}
              <div style={{width:"100%",height:`${h}%`,minHeight:d.count>0?6:2,background:d.count>0?"linear-gradient(to top,#2563eb,#60a5fa)":"var(--surface-2)",borderRadius:"4px 4px 0 0",transition:"height .5s ease"}}/>
            </div>);});})()}
        </div>
        <div style={{display:"flex",justifyContent:"space-between",marginTop:8,fontSize:10,color:"var(--text-3)",fontFamily:"var(--mono)"}}>
          <span>{dailyDays[0]?.date}</span><span>{dailyDays[dailyDays.length-1]?.date}</span>
        </div>
        {/* Debug: raw daily_activity keys from backend */}
        <details style={{marginTop:12}}>
          <summary style={{fontSize:10,color:"var(--text-3)",cursor:"pointer",fontFamily:"var(--mono)"}}>🔧 Debug: raw daily_activity data</summary>
          <pre style={{fontSize:9,fontFamily:"var(--mono)",color:"var(--text-3)",marginTop:4,maxHeight:120,overflow:"auto",background:"var(--surface-2)",padding:8,borderRadius:6}}>{JSON.stringify(data.daily_activity,null,2)}</pre>
        </details>
      </Card>
      ...
      <Card pad="24px" style={{background:"linear-gradient(135deg,#f8fafc,#f1f5f9)",border:"1px solid var(--border)"}}>
        <CardTitle icon="📋">Auto-VIA Platform Adoption Metrics</CardTitle>
        <p style={{fontSize:12,color:"var(--text-2)",marginBottom:16}}>Structured summary of platform usage metrics with downloadable report.{filterActive?" Showing filtered data for selected date range.":""}</p>
...
ADOPTION & REACH
  Total User Sessions:              ${(data.total_sessions||0).toLocaleString()}${(data.unique_visitors||Object.keys(data.visitor_ids||{}).length||0)>=20?`
  Unique Visitors:                  ${(Object.keys(data.visitor_ids||{}).length||(data.unique_visitors||0)).toLocaleString()}
  Returning User Sessions:          ${(data.returning_sessions||0).toLocaleString()} (${retentionPct.toFixed(1)}%)
  New User Sessions:                ${(data.new_sessions||0).toLocaleString()}
  Retention Rate:                   ${retentionPct.toFixed(1)}%`:""}
...
PLATFORM UTILIZATION
  Vulnerability Analyses Performed: ${(data.total_analyses||0).toLocaleString()}
  Unique CVEs Reviewed:             ${(uniqueCVEs||0).toLocaleString()}
  Live NVD Searches Executed:       ${(data.total_live_searches||0).toLocaleString()}
  AI-Powered Analysis Queries:      ${(data.total_ai_queries||0).toLocaleString()}
  Manual Risk Assessments:          ${(data.total_manual_assessments||0).toLocaleString()}${!filterActive?`\n  Peak CVEs Processed (single run): ${(analytics.peak_concurrent_avrs||0).toLocaleString()}`:""}
...
COMPLIANCE & EXPORT ACTIVITY
  Total Compliance Exports:         ${totalExports.toLocaleString()}
    ...
    ├─ CSV Data Exports:            ${(data.total_csv_exports||0).toLocaleString()}
    └─ JSON Data Exports:           ${(data.total_json_exports||0).toLocaleString()}
...
${featureEntries.filter(([,v])=>v>0).map(([f,c])=>{const labels={dashboard:"Dashboard",search:"Search & Triage",architecture:"Vehicle Architecture",ai:"AI Assistant",assess:"Manual Assessment",about:"Documentation",analytics:"Analytics"};return`  ${(labels[f]||f).padEnd(28)}${c.toLocaleString()} views`;}).join("\n")||"  (No feature engagement recorded yet)"}`
      ...
      <Card>
        <CardTitle icon="⚖️">Platform Health Benchmarks</CardTitle>
        <p style={{fontSize:12,color:"var(--text-2)",marginBottom:16}}>Auto-assessed strength of current platform metrics. Updates in real time as usage grows.</p>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {[
            {criterion:"Adoption (Users Engaging)",metric:`${data.total_sessions||0} sessions`,strength:(data.total_sessions||0)>=100?"strong":(data.total_sessions||0)>=20?"moderate":"building",tip:(data.total_sessions||0)<100?"Target 100+ sessions. Share with Auto-ISAC, ASRG, and LinkedIn automotive security communities.":"Strong session count. Continue building."},
            ...
            {criterion:"Utility Depth (Analyses/Exports)",metric:`${data.total_analyses||0} analyses, ${totalExports} exports`,strength:(data.total_analyses||0)>=500?"strong":(data.total_analyses||0)>=50?"moderate":"building",tip:(data.total_analyses||0)<500?"Target 500+ analyses. Each CVE view, search, and export counts. Encourage real workflow integration.":"Strong utilization depth."},
            {criterion:"AI-Powered Analysis",metric:`${data.total_ai_queries||0} AI queries`,strength:(data.total_ai_queries||0)>=50?"strong":(data.total_ai_queries||0)>=10?"moderate":"building",tip:"AI queries show the platform provides novel analytical capabilities beyond existing tools."},
            {criterion:"Compliance Outputs (TARA/AVR)",metric:`${data.total_tara_exports||0} TARA + ${data.total_avr_downloads||0} AVR exports`,strength:((data.total_tara_exports||0)+(data.total_avr_downloads||0))>=20?"strong":((data.total_tara_exports||0)+(data.total_avr_downloads||0))>=5?"moderate":"building",tip:"ISO/SAE 21434 compliance exports demonstrate direct regulatory value."},
            ...
            ...((data.unique_visitors||Object.keys(data.visitor_ids||{}).length||0)>=20?[{criterion:"User Retention (Returning Users)",metric:`${retentionPct.toFixed(0)}% return rate, ${Object.keys(data.visitor_ids||{}).length||(data.unique_visitors||0)} unique visitors`,strength:retentionPct>=30?"strong":retentionPct>=10?"moderate":"building",tip:retentionPct<30?"Target 30%+ returning users. Returning users prove the tool has lasting value, not just one-time curiosity. Encourage bookmarking and workflow integration.":"Strong returning user rate demonstrates sustained utility."}]:[]),
      ...
      <Card>
        ...
        <p style={{fontSize:12,color:"var(--text-2)",marginBottom:16}}>This analytics system uses two independent layers to establish that usage data is genuine and has not been fabricated or inflated.</p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:20}}>
          {[
            ["🔗","Layer 1: Centralized Analytics with Hash-Chained Audit Log","Every user event from every browser worldwide is sent to a central Upstash Redis database via serverless API. Each event is appended to a SHA-256 hash chain — if any entry is modified, inserted, or deleted, the chain breaks, providing cryptographic proof of data integrity.",analytics.audit_log?.length?`${analytics.audit_log.length} entries · Chain head: ${(analytics.audit_chain_head||"").slice(0,12)}…`:"Chain not yet started","#7c3aed","#f5f3ff"],
            ["📊","Layer 2: Google Analytics 4","GA4 (Measurement ID: G-L770P062V5) runs independently on Google's infrastructure via gtag.js injected at runtime. Every tracked event is dual-fired to both Layer 1 and GA4. The petitioner cannot modify GA4 data. Cross-referencing GA4 session counts, geographic data, and event counts with Layer 1 data proves consistency.","Compare GA4 dashboard screenshots with these metrics","#2563eb","#eff6ff"],
