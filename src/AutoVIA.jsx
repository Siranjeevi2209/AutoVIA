...
// API calls go through our Vercel serverless proxy (/api/proxy.js)
// This avoids CORS issues since NVD API blocks direct browser requests
...
const AUTOMOTIVE_NVD_SEARCHES = [
  ...
  // OEMs
  { keyword: "toyota vehicle", label: "Toyota" },
  { keyword: "tesla vehicle", label: "Tesla" },
  { keyword: "BMW vehicle vulnerability", label: "BMW" },
  { keyword: "mercedes benz vehicle", label: "Mercedes-Benz" },
  { keyword: "volkswagen vehicle", label: "Volkswagen" },
  { keyword: "ford vehicle vulnerability", label: "Ford" },
  { keyword: "hyundai vehicle", label: "Hyundai" },
  { keyword: "honda vehicle vulnerability", label: "Honda" },
  // Safety-critical & Access
  { keyword: "CAN injection", label: "CAN Injection" },
  { keyword: "keyless entry vehicle", label: "Keyless Entry" },
  { keyword: "relay attack vehicle", label: "Relay Attack" },
  { keyword: "key fob vulnerability", label: "Key Fob" },
  { keyword: "immobilizer vehicle", label: "Immobilizer" },
  { keyword: "tire pressure monitoring", label: "TPMS" },
  { keyword: "EV charging station", label: "EV Charging" },
  { keyword: "battery management vehicle", label: "Battery Management" },
  // ADAS
  { keyword: "autonomous driving vulnerability", label: "Autonomous Driving" },
  { keyword: "adaptive cruise control", label: "Adaptive Cruise" },
  { keyword: "lidar vehicle", label: "LiDAR" },
  // More suppliers
  { keyword: "aptiv", label: "Aptiv" },
  { keyword: "qualcomm automotive", label: "Qualcomm Auto" },
  { keyword: "valeo automotive", label: "Valeo" },
  // Protocols & connectivity
  { keyword: "OBD-II vulnerability", label: "OBD-II" },
  { keyword: "SOME/IP", label: "SOME/IP" },
  { keyword: "connected car vulnerability", label: "Connected Car" },
  { keyword: "fleet management vehicle", label: "Fleet Management" },
  { keyword: "geotab", label: "Geotab" },
  // Additional embedded
  { keyword: "threadx rtos", label: "ThreadX RTOS" },
  { keyword: "integrity rtos", label: "INTEGRITY RTOS" },
  { keyword: "automotive grade linux", label: "AGL" },
];
...
export default function AutoVIA() {
  ...
  const [kevSetRef, setKevSetRef] = useState(new Set());
  ...
  // ── DATA LOADING — Hybrid: Pre-built DB (instant) + Live API fallback ──
  useEffect(() => {
    let cancelled = false;
    ...
    async function loadData() {
      ...
      // Step 1: Try loading pre-built database (instant)
      addLog("Checking for pre-built CVE database...", "info");
      try {
        const dbResp = await fetch("/cve-database.json");
        if (dbResp.ok) {
          const db = await dbResp.json();
          if (db.vulnerabilities?.length > 0) {
            // Load KEV set for live search later
            try {
              const kevResp = await fetch(`${API_PROXY}?source=kev`);
              if (kevResp.ok) {
                const kevData = await kevResp.json();
                const ks = new Set();
                kevData.vulnerabilities?.forEach(v => ks.add(v.cveID));
                setKevSetRef(ks);
                setKevCount(ks.size);
                addLog(`KEV catalog loaded: ${ks.size} known exploited vulnerabilities`, "success");
              }
            } catch (e) { /* KEV fetch optional */ }

            // Load pre-built CVEs with ARS already computed
            const enriched = db.vulnerabilities.map(v => ({
              ...v,
              cvss_v4_base_score: v.cvss_base_score || v.cvss_v4_base_score || 0,
              ...computeARS({
                ...v,
                cvss_v4_base_score: v.cvss_base_score || v.cvss_v4_base_score || 0,
                cvss_v3_base_score: v.cvss_base_score || 0,
              }),
            }));
            ...
            if (!cancelled) {
              setVulns(enriched);
              setLoading(false);
              addLog(`Pre-built database loaded: ${enriched.length} automotive AVRs`, "success");
              addLog(`Database generated: ${db.generated_at}`, "info");
              addLog(`Pipeline status: READY — Use "Search NVD Live" for on-demand lookups`, "success");
            }
            return;
          }
        }
      } catch (e) {
        addLog("No pre-built database found, falling back to live API...", "warn");
      }

      // Step 2: Fallback — Live API ingestion (if no pre-built DB)
      ...
      try {
        ...
        addLog(`KEV fetch error — continuing without KEV data`, "warn");
      }
      ...
      for (const searchItem of AUTOMOTIVE_NVD_SEARCHES) {
        ...
        try {
          const url = `${API_PROXY}?source=nvd&keyword=${encodeURIComponent(searchItem.keyword)}&resultsPerPage=50`;
          const resp = await fetch(url);

          if (resp.status === 403 || resp.status === 429) {
            addLog(`Rate limited on "${searchItem.keyword}" — waiting 15s...`, "warn");
          ...
          for (const item of items) {
            ...
            if (!cveId || allAVRs.has(cveId)) continue;
            const avr = parseNVDtoAVR(item, kevSet);
            if (avr && avr.cvss_v4_base_score > 0) { allAVRs.set(cveId, avr); added++; }
          }
          addLog(`  → ${items.length} CVEs received, ${added} new AVRs created (${allAVRs.size} total)`, "success");
          if (!cancelled) setVulns(Array.from(allAVRs.values()).sort((a, b) => b.ars - a.ars));
      ...
      if (!cancelled) {
        ...
        setVulns(finalVulns);
        setLoading(false);
        addLog(`Ingestion complete: ${finalVulns.length} automotive AVRs`, "success");
      }
    }
  ...
  // Live NVD search — search for any CVE or keyword on demand
  const handleLiveNVDSearch = async (query) => {
    if (!query || query.length < 3 || liveSearching) return;
    setLiveSearching(true);
    addLog(`Live NVD search: "${query}"`, "info");

    try {
      // If it looks like a CVE ID, search by exact ID
      const isCVE = /^CVE-\d{4}-\d+$/i.test(query.trim());
      const searchTerm = isCVE ? query.trim() : query;
      
      const url = `${API_PROXY}?source=nvd&keyword=${encodeURIComponent(searchTerm)}&resultsPerPage=50`;
      const resp = await fetch(url);

      if (!resp.ok) {
        addLog(`Live search failed: ${resp.status}`, "error");
      ...
      setVulns(prev => {
        const existing = new Map(prev.map(v => [v.cve_id, v]));
        for (const item of items) {
          const cveId = item.cve?.id;
          if (!cveId || existing.has(cveId)) continue;
          const avr = parseNVDtoAVR(item, kevSetRef);
          if (avr && avr.cvss_v4_base_score > 0) {
            existing.set(cveId, avr);
            added++;
          }
        }
        return Array.from(existing.values()).sort((a, b) => b.ars - a.ars);
      });

      addLog(`Live search "${query}": ${items.length} CVEs found, ${added} new AVRs added`, "success");
    } catch (e) {
      addLog(`Live search error: ${e.message}`, "error");
    }
    setLiveSearching(false);
  };
        ...
        {view === "search" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", gap: 10, padding: 16, borderRadius: 10, background: "#0B1120", border: "1px solid #1E293B", flexWrap: "wrap" }}>
              ...
              <button onClick={() => handleLiveNVDSearch(search)} disabled={liveSearching || search.length < 3}
                style={{ ...selectStyle, background: liveSearching ? "#7C3AED20" : "#2563EB15", border: "1px solid #2563EB40", color: liveSearching ? "#A78BFA" : "#60A5FA", fontWeight: 700, opacity: search.length < 3 ? 0.4 : 1, cursor: search.length < 3 ? "not-allowed" : "pointer" }}>
                {liveSearching ? "⏳ Searching..." : "🔍 Search NVD Live"}
              </button>
