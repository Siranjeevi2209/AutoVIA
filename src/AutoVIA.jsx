  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        ...
        <StatBox ico="🔁" value={data.returning_sessions||0} label="Returning Sessions" color="#7c3aed" sub={`${(data.total_sessions||0)>0?((data.returning_sessions||0)/(data.total_sessions||1)*100).toFixed(0):0}% return rate`}/>
        <StatBox ico="🆕" value={data.new_sessions||0} label="New User Sessions" color="#2563eb" sub="First-time visitors"/>
        <StatBox ico="📈" value={(data.total_sessions||0)>0?((data.returning_sessions||0)/(data.total_sessions||1)*100).toFixed(0)+"%":"—"} label="Retention Rate" color={((data.returning_sessions||0)/(data.total_sessions||1))>=0.3?"#059669":"#d97706"} sub={((data.returning_sessions||0)/(data.total_sessions||1))>=0.3?"Strong stickiness":"Building engagement"}/>
      </div>
...
ADOPTION & REACH
  Total User Sessions:              ${(data.total_sessions||0).toLocaleString()}
  Unique Visitors:                  ${(Object.keys(data.visitor_ids||{}).length||(data.unique_visitors||0)).toLocaleString()}
  Returning User Sessions:          ${(data.returning_sessions||0).toLocaleString()} (${(data.total_sessions||0)>0?((data.returning_sessions||0)/(data.total_sessions||1)*100).toFixed(1):0}%)
  New User Sessions:                ${(data.new_sessions||0).toLocaleString()}
  Retention Rate:                   ${(data.total_sessions||0)>0?((data.returning_sessions||0)/(data.total_sessions||1)*100).toFixed(1):0}%
      ...
      <Card>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {[
            ...
            {criterion:"User Retention (Returning Users)",metric:`${((data.returning_sessions||0)/(data.total_sessions||1)*100).toFixed(0)}% return rate, ${Object.keys(data.visitor_ids||{}).length||(data.unique_visitors||0)} unique visitors`,strength:((data.returning_sessions||0)/(data.total_sessions||1))>=0.30?"strong":((data.returning_sessions||0)/(data.total_sessions||1))>=0.10?"moderate":"building",tip:((data.returning_sessions||0)/(data.total_sessions||1))<0.30?"Target 30%+ returning users. Returning users prove the tool has lasting value, not just one-time curiosity. Encourage bookmarking and workflow integration.":"Strong returning user rate demonstrates sustained utility."},
