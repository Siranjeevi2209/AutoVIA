...
// ── DETAIL PANEL ──────────────────────────────────────────────────
function DetailPanel({v,onClose,aiAnalysis}){
...
\`\`\``;
  ...
  return(
    <div style={{minHeight:"100vh",background:"var(--bg)",color:"var(--text-0)",fontFamily:"var(--font)"}}>
              ...
              return(<>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  {kevVulns.slice(0,10).map((v,i)=>(
                    <div key={v.cve_id} onClick={()=>setSelected(v)} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",borderRadius:10,background:"#fff1f2",border:"1px solid #fecdd3",cursor:"pointer",transition:"all .2s",animation:`fadeUp .3s ease ${i*.04}s both`}}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
            ...
            <Card>
              ...
              {vulns.filter(v=>v.priority_tier==="P0_critical"||v.priority_tier==="P1_high").slice(0,6).map((v,i)=>(
                <div key={v.cve_id} onClick={()=>setSelected(v)} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",borderRadius:10,border:"1px solid var(--border)",cursor:"pointer",transition:"all .2s",marginBottom:6,animation:`fadeUp .4s ease ${i*.04}s both`}}
      ...
      {selected&&<><div onClick={()=>setSelected(null)} style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,.3)",zIndex:999}}/><DetailPanel v={selected} onClose={()=>setSelected(null)} aiAnalysis={aiAnalysis}/></>}
