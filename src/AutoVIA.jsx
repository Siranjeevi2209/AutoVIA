...
function EcuChart({vulns}){const counts={};vulns.forEach(v=>{counts[v.ecu_domain]=(counts[v.ecu_domain]||0)+1;});const sorted=Object.entries(counts).sort((a,b)=>b[1]-a[1]);const max=Math.max(...sorted.map(([,c])=>c),1);
  return(<Card><CardTitle icon="🏎️">ECU Domains</CardTitle>{sorted.map(([d,c],i)=>{const e=ECU[d];const pct=(c/max)*100;const isNarrow=pct<15;return(<div key={d} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8,animation:`fadeUp .4s ease ${i*.04}s both`}}>
    ...
    <div style={{flex:1,display:"flex",alignItems:"center",gap:8}}>
      <div style={{flex:1,height:24,background:"var(--surface-2)",borderRadius:8,overflow:"hidden"}}>
        <div style={{height:"100%",width:`${pct}%`,background:`linear-gradient(90deg,${e?.color}30,${e?.color}10)`,borderRadius:8,transition:"width 1s cubic-bezier(.4,0,.2,1)",display:"flex",alignItems:"center",justifyContent:"flex-end",paddingRight:isNarrow?0:8}}>
          {!isNarrow&&<span style={{fontFamily:"var(--mono)",fontSize:11,fontWeight:700,color:e?.color}}>{c}</span>}
        </div>
      </div>
      {isNarrow&&<span style={{fontFamily:"var(--mono)",fontSize:11,fontWeight:700,color:e?.color,flexShrink:0}}>{c}</span>}
    </div>
  </div>);})}</Card>);
}

function PriorityChart({vulns}){const total=vulns.length||1;
  return(<Card><CardTitle icon="📋">Priority Distribution</CardTitle>{Object.entries(TIERS).map(([k,t],i)=>{const c=vulns.filter(v=>v.priority_tier===k).length,pct=((c/total)*100).toFixed(0);const isNarrow=+pct<12;return(
    <div key={k} style={{display:"flex",alignItems:"center",gap:10,marginBottom:10,animation:`fadeUp .4s ease ${i*.06}s both`}}>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",gap:8}}>
        <div style={{flex:1,height:28,background:"var(--surface-2)",borderRadius:8,overflow:"hidden",position:"relative"}}>
          ...
          {c>0&&!isNarrow&&<span style={{position:"absolute",left:8,top:"50%",transform:"translateY(-50%)",fontFamily:"var(--mono)",fontSize:12,fontWeight:700,color:t.color}}>{c}</span>}
        </div>
        {c>0&&isNarrow&&<span style={{fontFamily:"var(--mono)",fontSize:12,fontWeight:700,color:t.color,flexShrink:0}}>{c}</span>}
      </div>
    ...
    <div style={{minHeight:"100vh",background:"var(--bg)",color:"var(--text-0)",fontFamily:"var(--font)"}}>
      ...
      <header style={{padding:"0 32px",height:64,display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid var(--border)",background:"var(--surface)",position:"sticky",top:0,zIndex:100}}>
        ...
        <nav style={{display:"flex",gap:2}}>
          {[["dashboard","Dashboard","◫"],["search","Search & Triage","⌕"],["assess","Manual Assessment","＋"],["about","How It Works","ℹ"]].map(([id,label,ico])=>(
