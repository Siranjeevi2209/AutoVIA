          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div>
              ...
              <p style={{fontSize:13,color:"#94a3b8",lineHeight:1.7,maxWidth:600}}>
                Work-product activity maintained as cumulative platform counters with a rolling hash-chained audit window — ISO/SAE 21434 compliance exports, manual risk assessments, and analysis events by ECU domain generated using Auto-VIA.
              </p>
            </div>
...
VERIFICATION
  Cumulative totals below are maintained as server-side counters
  incremented at event time. The most recent events are retained in
  a fixed-size rolling audit window secured by a SHA-256 hash chain:
  each entry's hash is computed over its own content together with a
  prior chain hash, so altering any retained entry breaks hash
  recomputation.
  Retained audit entries (rolling window): ${(analytics.audit_log?.length||0).toLocaleString()}
...
${topECUs.length?topECUs.map(([d,c])=>`  ${(ECU[d]?.full||d).padEnd(28)}${c.toLocaleString()} events`).join("\n"):"  (No domain-specific analysis events recorded yet)"}`
      ...
      <Card>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:20}}>
          {[
            ["🔗","Layer 1: Centralized Analytics with Hash-Chained Audit Log","Every user event from every browser is sent to a central Upstash Redis database via serverless API. Each event is appended to a SHA-256 hash chain, and the most recent events are retained as a fixed-size rolling tamper-evident window; cumulative totals are maintained as counters incremented at event time. Altering any retained entry breaks hash recomputation.",analytics.audit_log?.length?`${analytics.audit_log.length} entries · Chain head: ${(analytics.audit_chain_head||"").slice(0,12)}…`:"Chain not yet started","#7c3aed","#f5f3ff"],
      ...
      <Card>
        ...
        <p style={{fontSize:12,color:"var(--text-2)",marginBottom:8}}>Each event's hash is computed over its own content together with a prior chain hash; events recorded in the same request batch share a parent hash. The most recent events are retained as a fixed-size rolling window, and altering any retained entry breaks hash recomputation.</p>
