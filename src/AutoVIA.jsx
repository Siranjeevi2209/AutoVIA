...
function useAnalytics(){
  ...
  useEffect(()=>{
    async function load(){
      try{
        const raw=localStorage.getItem("autovia-analytics");
        if(raw){
  ...
  const persist=useCallback(async(data)=>{
    ...
    try{localStorage.setItem("autovia-analytics",JSON.stringify(data));}catch{}
  },[]);
  ...
  const resetAnalytics=useCallback(async()=>{
    ...
    setAnalytics(fresh);
    try{localStorage.setItem("autovia-analytics",JSON.stringify(fresh));}catch{}
  },[]);
